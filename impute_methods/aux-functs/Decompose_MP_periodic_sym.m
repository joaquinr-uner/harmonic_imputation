function [IMF,theta,phi,dtheta]=Decompose_MP_periodic_sym(f,theta_ini)
...
for alpha=1/floor(M):1/floor(M):1/3 
    for step=1:10 
        
        % interpolate the data to the theta space
        h_t=(theta(end)-theta(1))/N_t;
        theta_fit=[theta(1):h_t:theta(end)-h_t]';
        ...
        dtheta=2*pi*round((theta(end)-theta(1))/(2*pi))/(theta(end)-theta(1))*dtheta-lambda*dtheta_add_s;
